...
document.getElementById('btn-reset').addEventListener('click', async function() {
  ...
  try {
    ...
    async def api_reset_password(request: Request, payload: Dict[str, Any] = {}):
        ...
        async with _email_users_lock:
            with transaction(immediate=True) as db:
                ...
                eu = db.execute("SELECT * FROM email_users WHERE email=?", (email,)).fetchone()
                if not eu:
                    raise HTTPException(404, "用户不存在")
                db.execute(
                    "UPDATE email_users SET password_hash=?, login_fails=0, login_fail_time=0 WHERE email=?",
    ...
    async def api_login_email(request: Request, payload: Dict[str, Any]):
        ...
        token = secrets.token_urlsafe(32)
        now = time.time()
        from web.app import _load_sessions, _sessions_lock
        async with _sessions_lock:
            ...
            main_db.save_session(token_hash, uid, now + SESSION_MAX_AGE_SEC, int(is_admin), "")
    ...
    async def api_change_password(request: Request, payload: Dict[str, Any] = {}):
        ...
        # 销毁该用户的所有旧 session
        uid = "email:" + email
        main_db.clear_sessions_for_user(uid)
        print(f'[auth] password changed: {email}')
        _secure = SITE_URL.startswith("https")
        resp = JSONResponse({"ok": True, "message": "密码修改成功"})
        resp.delete_cookie("session", path="/", httponly=True, samesite="lax", secure=_secure)
        return resp
    ...
    async def api_admin_email_users(request: Request, search: str = "",
        ...
        for email, eu in users.items():
            ...
            gid = "email:" + email
            role = eu.get("role", "user")
            # 如果主用户表标记为 admin，以主表为准
            if main_users.get(gid, {}).get("role") == "admin":
                role = "admin"
            items.append({
                "github_id": gid,
                "login": email.split("@")[0],
    ...
    async def api_admin_email_user_ban(request: Request, payload: Dict[str, Any] = {}):
        ...
        from web.app import _load_users as _load_main_users
        main_users = _load_main_users()
        if main_users.get(github_id, {}).get("role") == "admin":
