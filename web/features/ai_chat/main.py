        ...
        if do_search or gen_mode:
            ...
            if gen_mode:
                gen_sys_guide = (
                    ...
                    "8.5. **需求不明确时先讨论（仅限主体/氛围含糊时）**：\n"
                    "   - **需要反问的情况（才先讨论、不直接出卡）**：用户只给了画面主体但缺关键可生成信息、且信息缺失会根本改变画面——如：① 只说主体没说任何场景/风格/氛围（如'画一个女生'）；② 主体与场景都含糊到无法生成（如'画个奇怪的场景'）；③ 画风/角色库搜不到。此时先列出可选项/反问澄清（推荐 3-5 种风格供选择，或询问场景/角色/画幅偏好），给出建议方案，等用户确认后再生成卡片。\n"
                    "   - **直接出卡的情况（信息已足够明确）**：用户给了主体+场景/氛围（如'雨夜城市天台、清晨森林、海边日落'），即使没明确指定风格/画风——此时画面信息已足够生成，**不要反问**，直接调用 trigger_generation 生成卡片，风格按顶部工作流默认或推荐一个最合适的（写实/电影/动漫等）即可。\n"
