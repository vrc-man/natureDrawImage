...
async def gen_stats_generation(request: Request, date_from: float = 0, date_to: float = 0, login: str = "", tz_offset: float = 0):
    ...
    if date_from and date_to:
        return {
            ...
            "hourly": db.get_gen_logs_hourly_range(date_from, date_to, tz_offset=tz_offset, login=login),
            "daily": db.get_gen_logs_daily_range(date_from, date_to, tz_offset=tz_offset, login=login),
        }
    return {
