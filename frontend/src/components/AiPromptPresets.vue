...
const props = defineProps<{ onFill: (text: string) => void }>()
...
const STORAGE_KEY = 'aiChatPromptPresets'
...
function showEditForm(id: number) {
  ...
  if (!p) return
  editingId.value = id; formName.value = p.name; formText.value = p.content
  view.value = 'form'
}
function saveForm() {
  if (!formName.value.trim() || !formText.value.trim()) return
  if (editingId.value !== null) {
    presets.value[editingId.value] = { name: formName.value.trim(), content: formText.value.trim() }
  } else {
    presets.value.push({ name: formName.value.trim(), content: formText.value.trim() })
  }
...
function importFromBox() {
  const ta = document.getElementById('aiChatSysPromptTa') as HTMLTextAreaElement
...
function fillPreset(id: number) {
  ...
  if (p) { props.onFill(p.content); open.value = false }
}
function exportPresets() {
  const blob = new Blob([JSON.stringify(presets.value, null, 2)], { type: 'application/json' })
  const a = document.createElement('a')
  a.href = URL.createObjectURL(blob); a.download = 'ai_chat_prompts.json'; a.click()
}
      ...
      <div class="mx-4 w-full max-w-[calc(100vw_-_10px)] bg-white/95 backdrop-blur-xl border border-pink-100 rounded-3xl shadow-2xl shadow-pink-100/40 flex flex-col max-h-[calc(100vh-4rem)]" @click.stop>
        ...
        <template v-if="view === 'list'">
          <div class="flex items-center justify-between p-5 pb-3 border-b border-pink-100 shrink-0">
            <h3 class="text-lg font-bold text-gray-700">📚 AI 系统提示词预设</h3>
